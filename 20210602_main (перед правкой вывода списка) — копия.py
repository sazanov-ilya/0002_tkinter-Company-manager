...
class Companies(tk.Frame):
    ...
    def init_companies(self):
        ...
        # Treeview для списка
        self.companies_list = ttk.Treeview(frm_companies_content, columns=('id_company', 'company_name',
                                                                           'company_description'),
                                           height=10, show='headings')
        # Параметры к колонкам
        self.companies_list.column("id_company", width=40, anchor=tk.CENTER)
        self.companies_list.column("company_name", width=150, anchor=tk.CENTER)
        self.companies_list.column("company_description", width=355, anchor=tk.CENTER)
        # Названия колонкам
        self.companies_list.heading('id_company', text='ID')
        self.companies_list.heading('company_name', text='Наименование')
        self.companies_list.heading('company_description', text='Описание')
        # Вывод в главное окно с выравниванием по левой стороне
        self.companies_list.pack(fill="both", side='left', expand=True)

        # Полоса прокрутки
        scroll = tk.Scrollbar(frm_companies_content, command=self.companies_list.yview)
        scroll.pack(side=tk.RIGHT, fill=tk.Y)
        self.companies_list.configure(yscrollcommand=scroll.set)
    ...
    def companies_show(self):
        ...
        # Очистка таблицы
        [self.companies_list.delete(i) for i in self.companies_list.get_children()]
        # Перезагргузка таблицы
        [self.companies_list.insert('', 'end', values=row) for row in self.db.c.fetchall()]
